...
function showDeathScreen(g) {
  ...
  fetch(`${ANALYTICS_URL}/leaderboard?limit=10`)
    ...
    .then(entries => {
      ...
      // Names come from arbitrary players in the leaderboard backend —
      // escape before injecting via innerHTML or a name like
      // <img onerror=alert(1)> would XSS every other player's death screen.
      ...
      const rows = entries.map((e, i) => {
        ...
        return `<div class="lb-row${isYou ? ' lb-you' : ''}">
          ...
          <span class="lb-name">${escape(e.name)}</span>
